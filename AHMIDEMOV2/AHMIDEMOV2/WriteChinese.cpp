...
//		for (U8 j = 0; j < 4; j++)
//		{
//			*(buffer + i * 16 + j * 4) = (U16)((ziku16[font_addr * 8 + j * 2] >> 16) & 0xffff);
//			*(buffer + i * 16 + j * 4 + 1) = (U16)(ziku16[font_addr * 8 + j * 2] & 0xffff);
//			*(buffer + i * 16 + j * 4 + 2) = (U16)((ziku16[font_addr * 8 + j * 2 + 1] >> 16) & 0xffff);
//			*(buffer + i * 16 + j * 4 + 3) = (U16)(ziku16[font_addr * 8 + j * 2 + 1] & 0xffff);
...
void WriteChinese(string word, ROMInfo &rom_info, TileInfoMask &tileinfomask, U8 & RomAddr, U8 &TEXADD)
{
	 ...
	 U32 *bufferend = new U32[wordlength * 32];
	 for (U8 i = 0; i < wordlength; i++)
	 {
	 	U8 TEST = word[i * 2];
	 	U8 TEST2 = word[i * 2 + 1];
	 	U16 font_addr = (TEST - 0XA1)*(0xff - 0xa1) + (TEST2 - 0xa1);
	 	for (U8 j = 0; j < 32; j++)
	 		buffer[i * 32 + j] = ziku32[font_addr * 32 + j];
	 }
	for (U8 j = 0; j < wordlength; j++)
		for (U8 i = 0; i <32; i++)
		{
			*(bufferend + j * 32 + i) = *(buffer + ((i + j * 32) % wordlength) * 32 + (i + j * 32) / wordlength);
		}
	for (U8 i = 0; i < wordlength * 16; i++)
		*(rom_info.tex[RomAddr].texel + i) =  (U64)*(bufferend + i * 2) << 32  |   (U64)*(bufferend + i * 2 + 1);
	RomAddr++;
}