...
void ahmi::TexRaster(Tile* tile,Matrix* matrix,Texture* texture,U16 tex_width,U16 tex_height,U8 texture_ctrl,Color16 InitColor)
{
	...
	u0 = (matrix->A * tile->x + matrix->C * tile->y)/128 + matrix->E;
	v0 = (matrix->B * tile->x + matrix->D * tile->y)/128 + matrix->F;
	...
	case(0):
			...
			for(int i = 0;i < 1024;i++)
			{
				...
				x=i%32;
				y=i/32;
				u = u0 + (matrix->A * x + matrix->C * y) / 8;
				v = v0 + (matrix->B * x + matrix->D * y) / 8;
