...
void READTEX::GetTexSize(TileInfoMask &tileinfomask,unsigned char &TEXADD)//获取纹理的大小为之分配空间
{
	if (flag == 0)
	{
		char *ch = new char[4];
	...
	tileinfomask.tileinfomask1[TEXADD].flag = flag;
	tileinfomask.tileinfomask1[TEXADD].height = height;
	tileinfomask.tileinfomask1[TEXADD].width = width;
	tileinfomask.tileinfomask1[TEXADD].PixelLength = PixelLength;
	TEXADD++;
}
void READTEX::GetTexPixel(U64 *&rominfo)//将纹理读入到roninfo中
{
	int i = 0;
