...
void start()
{
	...
	//           int mask);
	//添加纯色纹理
	//ADDPurity(tileinfomask->tileinfomask1[TEXADD],
	//	        TEXADD,
	//	        int mask,
	//	        unsigned char r,
	//	        unsigned char g,
	...
	/********************************************/
	LoadROMinfo(PICNAME0, rom_info, tileinfomask->tileinfomask1[TEXADD], RomAddr, TEXADD, 1);
	MatrixGenerate MatrixGenerate;
	MatrixGenerate.Tritranslate(((S1_B_4)-100) << 4, ((S1_B_4)-100) << 4);//已测试
	MatrixGenerate.Triscale((S1_3_4)4 << 4, (S1_3_4)4 << 4);//本身不为负数 已测试
	MatrixGenerate.Trihorizontal(3);//本身不为负数 已测试
	MatrixGenerate.Trivertical(1);//本身不为负数 已测试
	MatrixGenerate.Trirotate((S16)25); //已测试
	MatrixGenerate.GetMatrix(tile_info->Matrix[TEXADD - 1],
		Matrixmask->Matrixmask1[TEXADD - 1],
		matrix[TEXADD - 1],
		TEXADD);//获取矩阵
	LoadROMinfo(PICNAME1, rom_info, tileinfomask->tileinfomask1[TEXADD], RomAddr, TEXADD, 1);
	GetMatrix(matrix[TEXADD - 1], 0);
	LoadROMinfo(PICNAME2, rom_info, tileinfomask->tileinfomask1[TEXADD], RomAddr, TEXADD, 0);
	GetMatrix(matrix[TEXADD - 1], 0);
